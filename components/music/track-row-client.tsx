...
import { recordLike, recordDownload, Achievement } from "@/lib/gamification";
import { useToast } from "@/hooks/use-toast";
import { AchievementToast } from "./achievement-toast";

interface TrackRowClientProps {
  track: YouTubeVideo;
  index?: number;
  showAlbum?: boolean;
...
export function TrackRowClient({ 
  track, 
  index, 
  onDownloadClick,
  downloadProgress = 0,
  ...
  const handleLike = useCallback(async () => {
    try {
      ...
      if (isLiked) {
        ...
        setIsLiked(false);
        if (user) await syncUnlikeTrack(track.id, { offline: true });
      } else {
        await likeTrack(track);
        setIsLiked(true);
        const { newAchievements } = recordLike();
        showAchievementToast(newAchievements);
        if (user) await syncLikeTrack(track as Song, { offline: true });
      }
    } catch (error) {
  ...
  const handleDownload = useCallback((e: React.MouseEvent) => {
    ...
    e.stopPropagation();
    onDownloadClick?.();
    const { newAchievements } = recordDownload();
    showAchievementToast(newAchievements);
  }, [onDownloadClick]);
    ...
    <>
      ...
      >
        ...
        <div className="w-6 sm:w-8 flex items-center justify-center shrink-0">
          <Button variant="ghost" size="icon" className="w-6 h-6 p-0 flex sm:hidden" onClick={handlePlay}>
            {isCurrentlyPlaying ? <Pause className="w-4 h-4" /> : <Play className="w-4 h-4 fill-current" />}
          </Button>
          <div className="hidden sm:flex items-center justify-center w-full h-full">
            {isHovered || isCurrentlyPlaying ? (
              <Button variant="ghost" size="icon" className="w-6 h-6 p-0" onClick={handlePlay}>
                {isCurrentlyPlaying ? <Pause className="w-4 h-4" /> : <Play className="w-4 h-4 fill-current" />}
              </Button>
            ) : (
              <span className={cn("text-xs sm:text-sm tabular-nums", isCurrentTrack ? "text-primary" : "text-muted-foreground")}>
                {index}
              </span>
            )}
          </div>
        ...
        <div className="w-8 sm:w-10 h-8 sm:h-10 rounded overflow-hidden bg-secondary shrink-0">
          {track.thumbnail ? (
            ...
            <div className="w-full h-full flex items-center justify-center bg-primary/10">
              <span className="text-xs sm:text-sm font-bold text-primary">{track.title[0]}</span>
            </div>
          )}
        </div>
        ...
        <div className="flex-1 min-w-0">
          <h3 className={cn("font-medium text-xs sm:text-sm truncate", isCurrentTrack ? "text-primary" : "text-foreground")}>
            {track.title}
          </h3>
        ...
        {isDownloading && (
          <div className="flex items-center gap-1 sm:gap-2 w-20 sm:w-24 shrink-0">
            <div className="flex-1 bg-secondary rounded-full h-1 overflow-hidden">
              <div className="bg-green-500 h-full" style={{ width: `${downloadProgress}%` }} />
            </div>
          ...
          <>
            {isDownloaded ? (
              <div className="flex items-center gap-1 sm:gap-2 shrink-0">
                ...
                <Button variant="ghost" size="icon" className="w-6 h-8 p-0 opacity-0 group-hover:opacity-100" onClick={handleRemoveDownload} title="Delete download">
                  <Trash2 className="w-4 h-4 text-red-500" />
                </Button>
              </div>
            ) : (
              <Button variant="ghost" size="icon" className="w-6 h-8 p-0 opacity-0 group-hover:opacity-100" onClick={handleDownload} title="Download" disabled={isDownloading}>
        ...
        <div>
          <DropdownMenu>
            <DropdownMenuTrigger asChild>
              <Button variant="ghost" size="icon" className="w-6 sm:w-8 h-6 sm:h-8 p-0 opacity-100 sm:opacity-0 sm:group-hover:opacity-100">
                <MoreHorizontal className="w-3 sm:w-4 h-3 sm:h-4" />
              </Button>
            </DropdownMenuTrigger>
            <DropdownMenuContent align="end" className="w-48">
              <DropdownMenuItem onClick={() => addToQueue(track as Song)}><ListPlus className="w-4 h-4 mr-2" />Add to queue</DropdownMenuItem>
              <DropdownMenuItem onClick={(e) => { e.preventDefault(); e.stopPropagation(); setShowAddToPlaylist(true); }}><Plus className="w-4 h-4 mr-2" />Add to Playlist</DropdownMenuItem>
              <DropdownMenuItem onClick={handleLike}><Heart className="w-4 h-4 mr-2" />{isLiked ? "Remove from Liked" : "Add to Liked"}</DropdownMenuItem>
              {isDownloaded ? (
                <DropdownMenuItem onClick={handleRemoveDownload}><Trash2 className="w-4 h-4 mr-2" />Delete download</DropdownMenuItem>
              ) : (
                <DropdownMenuItem onClick={handleDownload} disabled={isDownloading}><Download className="w-4 h-4 mr-2" />{isDownloading ? "Downloading..." : "Save offline"}</DropdownMenuItem>
              )}
            </DropdownMenuContent>
          </DropdownMenu>
        </div>

        <AddToPlaylistDialog open={showAddToPlaylist} onOpenChange={setShowAddToPlaylist} song={track as Song} onAddSuccess={onAddToPlaylistSuccess} />
      </div>
    </>
  );
}
